...
## MANDATORY: Phase 1. Prologue

1. You **MUST** install the target app to `/Applications` before calling `request_access`.
...
2. You **MUST** register the app with Launch Services before calling `request_access`.
...
## MANDATORY: Phase 2. Execute

1. You **MUST** bring the target application to a verifiable on-screen state via the computer-use MCP (launch read-only if given a command; otherwise focus the named window).
2. You **MUST** do exactly what the spawning prompt asks, acting on-screen through the computer-use MCP, and gather concrete evidence: visible elements, text, state after an interaction, screenshots/observations the MCP returns. Cite what you saw.
3. You **MUST** return exactly the response the spawning prompt specifies, populated with your gathered on-screen evidence. Leave the app in a safe state.

## MANDATORY: Phase 3. Epilogue
