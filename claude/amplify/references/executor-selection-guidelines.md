...
When the **audit-resolver** selects a gated executor (any driver or external agent), it learns availability by running `variable --id <GRAPH_ID>` itself (using the injected GRAPH_ID) — **not** from the `$AMPLIFY_*` session flags, which it cannot see. The flag-based guards above govern the planning and implementer contexts.
