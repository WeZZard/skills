...
// Resolve plugin root: bundle lives at configurator/configurator.mjs, so root is ../
const PLUGIN_ROOT =
    ...
    for (const entry of eventHooks) {
      ...
      if (entryMatcher === moment.matcher || (moment.matcher === null && !entryMatcher)) {
        ...
        for (const cmd of cmds) {
          ...
          if (match) { config[moment.id] = match[1]; break; }
        }
        break;
      }
    }
...
function buildHooksJson(config: Record<string, string | null>): object {
...
async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (url.pathname === "/api/config" && req.method === "POST") {
    try {
      ...
      const payload = JSON.parse(body) as Record<string, string | null>;
